...
class Renderer():
    def __init__(self, cfgs, im_size=64):
        ...
        self.anti_aliasing = cfgs.get('anti_aliasing', True)
        ...
        rot_mat = [[[1., 0., 0.],
              [0., 1., 0.],
              [0., 0., 1.]]]
        rot_mat = torch.FloatTensor(rot_mat).to(self.device)
        ...
        k_mat = [[fx, 0., cx],
             [0., fy, cy],
             [0., 0., 1.]]
        k_mat = torch.FloatTensor(k_mat).to(self.device)
        self.inv_k_mat = torch.inverse(k_mat).unsqueeze(0)
        self.k_mat = k_mat.unsqueeze(0)
        self.renderer = nr.Renderer(camera_mode='projection',
                                    anti_aliasing=self.anti_aliasing,
                                    light_intensity_ambient=1.0,
                                    light_intensity_directional=0.,
                                    K=self.k_mat, R=rot_mat, t=t,
    ...
    def depth_to_3d_grid(self, depth):
        ...
        grid_3d = grid_3d.matmul(self.inv_k_mat.to(depth.device).transpose(2, 1)) * depth
        return grid_3d

    def grid_3d_to_2d(self, grid_3d):
        _, h, w, _ = grid_3d.shape
        grid_2d = grid_3d / grid_3d[..., 2:]
        grid_2d = grid_2d.matmul(self.k_mat.to(grid_3d.device).transpose(2, 1))[:, :, :, :2]
